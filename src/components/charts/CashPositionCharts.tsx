...
import { useCashData, fmtCash, fmtCashK } from '@/lib/hooks/useCashData'
import { computeArMissCashProjection } from '@/lib/hypotheticals/scenarios'

const BURN_GUIDANCE = 231000
...
export default function CashPositionCharts({ arDelayDays = 0, arOverdue = 186000 }: Props) {
    ...
    <div className="border-b border-border">
      <div className="px-4 pt-4 pb-2 grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card className="border-border bg-card px-3 py-2.5">
          ...
          <p className="text-xl font-semibold tabular-nums">{fmtCash(Number(latest?.total_cash))}</p>
          <p className="text-[11px] text-muted-foreground">
            Operating {fmtCash(Number(latest?.operating))} · Reserve {fmtCash(Number(latest?.reserve))}
          </p>
        </Card>
        <Card className={`border-border bg-card px-3 py-2.5 ${burnStatus === 'red' ? 'border-l-2 border-l-red-500' : burnStatus === 'yellow' ? 'border-l-2 border-l-yellow-400' : ''}`}>
          ...
          <div className="flex items-baseline gap-2">
            <p className="text-xl font-semibold tabular-nums">{fmtCashK(avgBurn)}</p>
          ...
          <p className="text-[11px] text-muted-foreground">Guidance: {fmtCashK(BURN_GUIDANCE)}/wk</p>
        </Card>
        <Card className="border-border bg-card px-3 py-2.5">
        ...
        <Card className="border-border bg-card px-3 py-2.5">
          ...
          <p className="text-xl font-semibold tabular-nums">
            {prev && latest ? fmtCash(Math.abs(Number(latest.total_cash) - Number(prev.total_cash))) : '—'}
          </p>
          <p className="text-[11px] text-muted-foreground">
            {prev && latest
      ...
      <div className="px-4 pb-1">
        <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium mb-2">
          8-Week Cash Balance{arDelayDays > 0 ? ' · AR delay scenario overlay' : ''}
        </p>
        <ResponsiveContainer width="100%" height={180}>
          <AreaChart data={chartData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
            ...
            <YAxis tickFormatter={v => `$${(Number(v) / 1e6).toFixed(1)}M`} tick={{ fontSize: 10, fill: '#9ca3af' }} width={56} />
            <Tooltip
            ...
            {arDelayDays > 0 && (
              <Area type="monotone" dataKey="arMiss" stroke="#ef4444" fill="none" strokeWidth={1.5} strokeDasharray="4 2" dot={false} name="If AR delayed" />
            )}
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="px-4 pb-4">
        ...
        <ResponsiveContainer width="100%" height={140}>
          <BarChart data={chartData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
            <XAxis dataKey="week" tick={{ fontSize: 10, fill: '#9ca3af' }} />
            <YAxis tickFormatter={v => fmtCashK(Number(v))} tick={{ fontSize: 10, fill: '#9ca3af' }} width={56} />
            <Tooltip
              contentStyle={{ background: '#1f2937', border: '1px solid #374151', borderRadius: 6, fontSize: 12 }}
              formatter={(v: unknown) => [fmtCashK(Number(v)), 'Weekly Burn']}
            />
            <ReferenceLine y={BURN_GUIDANCE} stroke="#facc15" strokeDasharray="4 2" label={{ value: `Guidance ${fmtCashK(BURN_GUIDANCE)}`, fontSize: 10, fill: '#facc15', position: 'insideTopRight' }} />
